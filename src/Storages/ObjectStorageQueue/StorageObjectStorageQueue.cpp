...
StorageObjectStorageQueue::StorageObjectStorageQueue(
    ...
    , zk_path(chooseZooKeeperPath(getContext(), table_id_, context_->getSettingsRef(), *queue_settings_).path)
    ...
    , keep_data_in_keeper(keep_data_in_keeper_)
{
    ...
    auto table_metadata = ObjectStorageQueueMetadata::syncWithKeeper(
        zk_path, *queue_settings_, storage_metadata.getColumns(), configuration_->format, context_, keeper_name, is_attach, log);
...
void StorageObjectStorageQueue::shutdown(bool is_drop)
{
    ...
    if (files_metadata)
    {
        ...
        ObjectStorageQueueMetadataFactory::instance().remove(zk_path, keeper_name, getStorageID(), is_drop, keep_data_in_keeper);
...
StorageObjectStorageQueue::KeeperPathWithName StorageObjectStorageQueue::chooseZooKeeperPath(
    const ContextPtr & context_,
    ...
    UUID database_uuid)
{
    ...
    KeeperPathWithName result;
    result.keeper_name = zkutil::extractZooKeeperName(result_zk_path);
    if (result.keeper_name.empty())
        result.keeper_name = "default";
    result.path = zkutil::extractZooKeeperPath(result_zk_path, true);
    return result;
}
